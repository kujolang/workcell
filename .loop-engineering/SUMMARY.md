...
- Added 78 policy/artifact/verification assertions, 8 workspace assertions, a 200-file default performance signal, and an opt-in 2,000-file signal (32,259 ms on the current macOS host).
...
- Completed the enterprise-readiness review: changed-file analysis is reused within a run, human/JSON cleanup output is backend-neutral with a compatibility alias, RunLedger finish failures are explicit, the doctor uses the validated temporary root and selected runtime host variable, and all Kujo example-image pins match `RUNTIME_VERSION`.

## Verification

- passed: Kujo checks, 78 policy/artifact/verification assertions, 8 workspace assertions, performance signals, path-safety, adversarial rejection, dry-run, custom-network, custom security profiles, receipt redaction, integration command redaction, duplicate-input validation, ownership-marker validation, explicit dry-run integration skips, RunLedger finish-failure reporting, backend-aware CLI output, example validation, CLI schema/help smoke, version consistency, Docker integration including verification and clean inventory, and Docker/Colima doctor.
- passed: Kujo full test suite after the pushed runtime change, pinned Kujo example-image build, required docs/CLI/diagnostic integration contracts, VM 145/145, dual 145/145, Workcell offline suite, Docker integration with streamed logs, Docker and missing-Podman doctor diagnostics, Kennel, ShipCheck, ChangeBucket, and Fence with zero boundary violations.
