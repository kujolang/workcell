...
import { credential, handle, packageBytes, shellCommand, shellQuote, writeDownloaded } from './protocol.mjs';
...
export const vercel = {
  ...
  async execute(req) { const sb = await vercelConnect(req); const result = await sb.runCommand({ cmd: req.payload.argv[0], args: req.payload.argv.slice(1), cwd: req.payload.workdir, env: req.payload.environment, timeoutMs: req.payload.timeout_ms }); return commandData(req, result.exitCode, await result.stdout(), await result.stderr()); },
  async collect() { return collected(); },
...
export const daytona = {
  ...
  async execute(req) { const { sandbox } = await daytonaConnect(req); const result = await sandbox.process.executeCommand(shellCommand(req.payload.argv), req.payload.workdir, req.payload.environment, Math.ceil(req.payload.timeout_ms / 1000)); return commandData(req, result.exitCode, result.result || '', ''); },
  async collect() { return collected(); },
