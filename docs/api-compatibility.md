...
| Inspect summary | `workcell-inspect-summary/v1` | `inspect --summary` | Compact backend, policy, profile, and capability counts without workload command, mounts, secrets, or the full controls ledger. Full `inspect --json` remains authoritative. |
...
| Caller context | `workcell-caller-context/v1` | `run --context` input and portable receipt | Optional strict invocation correlation. Unknown fields are rejected; values are bounded non-secret identifiers. |
