...
The portable Docker vertical slice is also explicit: run `WORKCELL_LIVE_PORTABLE_OCI=1 KUJO=/path/to/kujo tests/portable_oci_contract.sh` only on an approved host with the fixture image available. Without the gate it reports a skip, preserving offline normal development.
