...
- Offline tests: `KUJO=../kujo/target/release/kujo ./tests/run.sh` — passed 80 definition/policy/artifact/verification assertions plus 8 workspace patch/change-report assertions, including path-safety, dry-run lifecycle, manifest verification/tamper detection, adversarial request rejection, custom-network, custom security profiles, duplicate-input validation, receipt redaction, partial-default, empty-secret, base64-derived-secret, binary-report, digest, signature-key, namespace, workspace ownership/scan, schema/help, backend-aware CLI output, RunLedger finish failure reporting, verification command redaction, optional integration evidence, explicit dry-run integration skips, and null-output regressions.
- CLI smoke: `./bin/workcell help`, `init`, `validate`, and `inspect --json` — passed.
- Docker integration: `TMPDIR=/var/folders/... KUJO=../kujo/target/release/kujo ./tests/docker_integration.sh` — passed against Docker server 29.5.2 through Colima, including success, streamed stdout/stderr log files, explicit non-cancelled receipt metadata, edit, failure, timeout, symlink, and label-scoped cleanup scenarios.
