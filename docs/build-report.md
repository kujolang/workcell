...
- Offline tests: `KUJO=../kujo/target/release/kujo ./tests/run.sh` — passed 194 definition/policy/artifact/verification assertions, 27 workspace patch/change-report/cleanup/scan assertions, and 7 deterministic stress assertions, including fail-closed workspace strategy/run-ID and CLI token validation plus malformed secret-section and image-ensure option checks.
- Structured report: `KUJO=../kujo/target/release/kujo ./tests/release_report.sh` — passed with `workcell-report/v1`, 228 counted assertions, per-suite elapsed time/exit codes, and explicit skipped Docker/Podman/ecosystem deployment gates.
