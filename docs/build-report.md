...
- Offline tests: `KUJO=../kujo/target/release/kujo ./tests/run.sh` — passed 74 definition/policy/artifact/verification assertions plus 8 workspace patch/change-report assertions, including path-safety, dry-run lifecycle, custom-network, custom security profiles, duplicate-input validation, receipt redaction, partial-default, empty-secret, base64-derived-secret, binary-report, digest, signature-key, namespace, workspace ownership/scan, schema/help, verification command redaction, optional integration evidence, and null-output regressions.
