...
The repository workflow is configured to run the offline, Docker, rootless-capable Podman, egress, concurrent-load, OCI, and doctor gates on `ubuntu-latest`. The latest observed hosted attempt was commit `a624c593d96e8eff9cfd5cbe2f66bdd05994935e` in GitHub Actions run [29245126661](https://github.com/kujolang/workcell/actions/runs/29245126661), but the `verify` job was not started: GitHub reported that recent account payments failed or the spending limit must be increased.

This is an external billing/Actions availability failure, not a test or source failure. Until repository billing is restored, hosted matrix evidence is unavailable and Workcell must not claim that hosted CI has passed. Local evidence remains current: 194 Workcell assertions, 27 workspace assertions, 7 stress assertions, 228 release assertions with 0 failures, Docker integration, Docker egress, concurrent Docker load, and Docker OCI smoke all passed.
