...
| Linux, rootless Podman | Set `runtime.backend: podman` and `workspace.run_as: rootless`; Podman's user namespace maps container `0:0` to the unprivileged engine user. | Rootless status is reported by `doctor`; policy, network, resource, provenance, and ownership contracts are shared with Docker. Host kernel/engine trust still applies. | Podman uses its native build path; image IDs are normalized into Workcell's `sha256:` receipt contract. | Supported when rootless mappings and the selected Podman features are available and doctor/integration checks pass. |
