...
`tests/quality.sh` is the shared quality gate for syntax-preserving Kujo formatting, zero source lint findings, shell syntax, and `git diff --check`; `tests/run.sh` and CI both execute it.
