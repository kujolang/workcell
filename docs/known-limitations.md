...
- Secrets are redacted from known exact values and common base64 encodings in completion-captured logs, verification output, and receipts; arbitrary hashed output still requires workload-level controls. Artifact export can explicitly allow, reject, or redact declared secret values.
...
- True incremental stream redaction and user-signal cancellation require Kujo process callbacks that are not yet exposed to Workcell; timeout and startup cleanup remain bounded and label-scoped.
