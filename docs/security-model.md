...
Preserved failed workspaces and handoff bundles can retain compromised code, prompts, customer data, and transformed secrets. `retain_until` records operator policy; it is not proof that a local or remote provider erased state at that time. Preservation outcomes therefore identify the cleanup owner and handle, and exported evidence must be inspected and redacted before it crosses a trust boundary. Workcell never infers live process or memory preservation from a retained filesystem.
