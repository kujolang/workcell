...
- The current offline suite passed 121 definition/policy/artifact/verification assertions, 11 workspace patch/change-report/cleanup/scan assertions, and 5 stress assertions; the adversarial suite, 200-file performance signal, malformed performance-environment input, exported policy/summary/daemon-security/verification/integration/image-build/ensure/receipt/container-policy partial-input checks, example definitions, schema/help, schema compatibility, release-report, CLI smoke, and version-consistency fixtures also passed.
