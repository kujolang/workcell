...
- `environment.allow`: host environment names explicitly passed into the container with Docker's `--env NAME` form. Host-control variables such as `PATH`, `HOME`, Docker/Podman selectors, cloud credential selectors, Kubernetes connection variables, Git configuration overrides, and package/CI credential variables are rejected.
...
- `artifacts.limits`: optional per-export path limits for `max_bytes`, `max_files`, and `max_depth`. Global `max_bytes`, `max_files`, and `max_depth` bound the complete export; `allowed_extensions` provides a simple content-type policy; `secret_action` may `allow`, `reject`, or `redact` declared secret values before export. `secret_action` also governs the generated `changes.patch`: the patch is always redacted regardless of this setting, and `reject` additionally fails the run and withholds the patch when it held a declared secret.
